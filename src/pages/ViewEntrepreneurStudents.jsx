...
function ViewEntrepreneurStudents() {
  ...
  useEffect(() => {
    if (facultyAdvisorName) {
      ...
      fetch(
        `${api_url}server/get_entrepreneur_student_details.php?advisor=${facultyAdvisorName}&batch=${batch}`,
        {
          method: "GET",
          credentials: "include",
        }
  ...
  const handleFileChange = (index, event) => {
    ...
    formData.append(
      "registerNumber",
      entrepreneurStudents[index].registerNumber
    );
    ...
    fetch(`${api_url}server/delete_alternative.php`, {
      ...
      .then((data) => {
        if (data.status === "success") {
          ...
          const newFileStates = [...fileStates];
          const index = entrepreneurStudents.findIndex(
      ...
      <div>
        ...
        <TableContainer className="fa-students-table" component={Paper}>
          <Table>
            ...
            <TableBody>
              {entrepreneurStudents.map((student, index) => (
...
export default ViewEntrepreneurStudents;
