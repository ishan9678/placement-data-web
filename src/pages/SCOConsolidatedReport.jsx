...
function SCOConsolidatedReport() {
  ...
  useEffect(() => {
    ...
    fetch(`${api_url}server/get_sco_consolidated_report.php?batch=${batch}`, {
      method: "GET",
    })
      .then((response) => response.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `${api_url}server/get_sco_all_department_statistics.php?batch=${batch}`,
      {
        method: "GET",
      }
    )
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          >
            Consolidated Report For All departments {batch}
          </h2>
          <div>
          ...
          {shouldRender && (
            <SalaryCategorisation
              apiUrl={`${api_url}server/get_sco_salary_categorisation.php?batch=${batch}`}
            />
          )}
          {/*Marquee*/}
          {shouldRender && (
            <MarqueeStudents
              apiUrl={`${api_url}/server/get_sco_marquee_students.php?batch=${batch}`}
            />
          )}
          {/*Offer Summary*/}
          ...
          {shouldRender && (
            <StudentStatistics
              apiUrl={`${api_url}server/get_sco_student_statistics.php?batch=${batch}`}
            />
          )}
          {/*Salary Statistics */}
          {shouldRender && (
            <SalaryStatistics
              apiUrl={`${api_url}server/get_sco_salary_statistics.php?batch=${batch}`}
            />
          )}
          {/*Company Statistics */}
          <CompanyStatistics
            apiUrl={`${api_url}server/get_sco_company_statistics.php?batch=${batch}`}
          />
        </div>
        <Button
...
export default SCOConsolidatedReport;
